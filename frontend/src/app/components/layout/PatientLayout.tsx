...
import { Home, Calendar, Clock, FileText, UserCircle, Activity, LogOut, ChevronLeft, ChevronRight, Bell, Search } from "lucide-react";
...
export default function PatientLayout() {
  ...
  const pageTitle = currentNav?.label || "Patient Portal";
    ...
    <div className="flex h-screen overflow-hidden bg-background font-sans">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 flex justify-center pt-3 px-4 pointer-events-none">
        <motion.div
          ...
          animate={{
            width: islandExpanded ? "92%" : scrolled ? 170 : 210,
            ...
            {islandExpanded && (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="avatar-chip bg-blue-100 text-blue-700 w-10 h-10 text-sm">{initials}</div>
                  <div>
                    <p className="text-white font-semibold text-sm">Hello, {user?.name?.split(" ")[0] ?? "Patient"}</p>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-5 h-[68px] border-b border-white/5">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 bg-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.35)]">
            <Activity className="w-5 h-5 text-white" />
          </div>
          <AnimatePresence>
            {!collapsed && (
              <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.18 }}>
                <p className="text-white font-bold text-base leading-none whitespace-nowrap">evodoc<span className="text-xs font-normal align-top ml-0.5 text-blue-400">®</span></p>
                <p className="text-gray-400 text-[11px] mt-0.5 whitespace-nowrap">Patient Portal</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        <nav className="flex-1 px-3 pt-4 pb-4 space-y-1.5 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={`w-[18px] h-[18px] flex-shrink-0 ${isActive ? "text-blue-400" : "text-gray-400 group-hover:text-gray-200"}`} strokeWidth={isActive ? 2.5 : 2} />
                <AnimatePresence>
                  {!collapsed && (
        ...
        {!collapsed && (
          <div className="mx-3 mb-3 p-3 rounded-2xl bg-white/[0.04] border border-white/5">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[11px] font-semibold text-gray-300">OPD Queue Online</span>
            </div>
            <p className="text-[10px] text-gray-500">Live token telemetry active</p>
          </div>
        )}
        ...
        <header className="hidden md:flex h-[68px] items-center justify-between px-8 bg-white border-b border-gray-100 flex-shrink-0 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2.5">
              <div className="avatar-chip bg-blue-100 text-blue-700 text-xs">{initials}</div>
              {!collapsed && (
                <div>
                  <p className="text-gray-800 font-semibold text-sm leading-none">{user?.name?.split(" ")[0] ?? "Patient"}</p>
                  <p className="text-gray-400 text-[11px] mt-0.5">Patient</p>
                </div>
              )}
            </div>
          </div>
